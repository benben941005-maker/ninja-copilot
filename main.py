...
# OneMap SG — uses email/password to fetch a JWT token (valid ~3 days)
ONEMAP_EMAIL    = os.environ.get("ONEMAP_EMAIL", "")
ONEMAP_PASSWORD = os.environ.get("ONEMAP_PASSWORD", "")

_onemap_token        = None   # cached JWT
_onemap_token_expiry = 0      # unix timestamp

def get_onemap_token():
    """Return a valid OneMap JWT, refreshing if expired or missing."""
    ...
    try:
        ...
        if not token:
            return None
        _onemap_token        = token
        _onemap_token_expiry = time.time() + 3500  # ~58 min safety margin
        return _onemap_token
    except Exception:
        return None
...
def geocode():
    ...
    Uses OneMap SG (official SLA geocoder) as primary when ONEMAP_EMAIL/PASSWORD are set,
...
# =========================================================
# PLACE SEARCH (Google Places → OSM fallback)
# =========================================================
@app.route("/api/place-search")
def place_search():
    """Search for nearby POI: petrol station, toilet, MRT, etc."""
    try:
        ...
        resp = requests.get(
            "https://nominatim.openstreetmap.org/search",
            params={
                "q":             f"{q} near Singapore",
                "format":        "json",
                "limit":         5,
                "countrycodes":  "sg",
                "viewbox":       "103.6,1.16,104.05,1.48",
                "bounded":       1,
            },
            headers={"User-Agent": "NinjaCoPilot/1.0"},
            timeout=8,
        ...
        # Pick closest to driver
        def dist(item):
            import math
            dlat = float(item["lat"]) - float(user_lat)
            dlng = float(item["lon"]) - float(user_lng)
            return math.sqrt(dlat*dlat + dlng*dlng)

        results.sort(key=dist)
        best = results[0]
        return jsonify({
            "name":    best.get("display_name", q).split(",")[0],
            "address": best.get("display_name", q),
            "lat":     float(best["lat"]),
            "lng":     float(best["lon"]),
            "source":  "nominatim",
        })
